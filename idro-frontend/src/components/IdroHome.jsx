...
import {
  ...
  Users,
  X,
  Truck, // Added for Response Modal
...
export default function IdroHome() {
  ...
  const [showResponseModal, setShowResponseModal] = useState(false); // NEW STATE
  ...
  <div className="max-w-[1920px] mx-auto px-6">
    <div className="flex justify-between items-center h-16">
      ...
      <nav className="hidden md:flex gap-2">
        ...
        <Link to="/command" className="flex items-center gap-2 px-4 py-2 rounded-md text-xs font-bold text-slate-300 hover:text-white hover:bg-white/5 transition-all">
          <LayoutDashboard size={14}/> Donation box
        </Link>
      </nav>
  ...
  <div>
    ...
    <p className="text-[10px] text-yellow-300 font-mono">
      AI AUTO-ALLOCATION
            ...
            {/* Card 4: AI */}
            <div className="bg-[#102a1c] rounded-lg p-4 border border-green-500/30 flex items-center gap-4 hover:border-green-500 transition-all">
  ...
  <div key={alert.id} className="bg-[#0f172a] p-4 rounded-xl border border-white/5 hover:border-orange-500/50 transition-all flex justify-between items-center group">
    <div>
      <div className="flex items-center gap-3 mb-1">
        ...
        <span className="text-xs text-slate-500 font-mono">ID: {alert.id.substring(0,8)}</span>
      </div>
    ...
    <div className="text-right">
      <button className="text-[10px] font-bold bg-orange-600 hover:bg-orange-500 text-white px-6 py-3 rounded uppercase tracking-wider shadow-lg flex items-center gap-2 transition-all hover:scale-105">
        <Siren size={14}/> DEPLOY UNITS
      </button>
    </div>
  </div>
))} */}
