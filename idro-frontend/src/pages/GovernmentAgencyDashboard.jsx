...
export default function GovernmentAgencyDashboard() {
    ...
    const ResourceCheckbox = ({ category, resourceName, index }) => {
        ...
        const handleCheckboxChange = (checked) => {
            setResources(prev => {
                const categoryResources = [...(prev[category] || [])];
                const existingIndex = categoryResources.findIndex(r => r.name === resourceName);

                if (existingIndex >= 0) {
                    categoryResources[existingIndex] = {
                        ...categoryResources[existingIndex],
                        available: checked
                    };
                } else {
                    categoryResources.push({ name: resourceName, available: checked, quantity: 0 });
                }
        ...
        const handleQuantityChange = (value) => {
            setResources(prev => {
                ...
                if (existingIndex >= 0) {
                    ...
                    categoryResources.push({ name: resourceName, available: false, quantity: parseInt(value) || 0 });
                }
                            ...
                            <div className="bg-[#1e293b] border border-white/10 rounded-xl p-6 shadow-xl">
                                ...
                                <div className="space-y-3">
                                    {["Rescue Personnel", "Boats", "Rescue Vehicles", "Diving Teams", "Heavy Equipment"].map((resourceName, index) => (
                                        <ResourceCheckbox key={index} category="NDRF_RESOURCES" resourceName={resourceName} index={index} />
                                    ))}
                                </div>
                            </div>
                        )}
                            ...
                            <div className="bg-[#1e293b] border border-white/10 rounded-xl p-6 shadow-xl">
                                ...
                                <div className="space-y-3">
                                    {["Doctors", "Nurses", "Ambulances", "First Aid Kits", "Mobile Medical Units"].map((resourceName, index) => (
                                        <ResourceCheckbox key={index} category="MEDICAL_RESOURCES" resourceName={resourceName} index={index} />
                                    ))}
                                </div>
                            </div>
                        )}
                            ...
                            <div className="bg-[#1e293b] border border-white/10 rounded-xl p-6 shadow-xl">
                                ...
                                <div className="space-y-3">
                                    {["Fire Engines", "Firefighters", "Hydraulic Cutters", "Ladders", "Foam Units"].map((resourceName, index) => (
                                        <ResourceCheckbox key={index} category="FIRE_RESOURCES" resourceName={resourceName} index={index} />
                                    ))}
                                </div>
                            </div>
                        )}
                            ...
                            <div className="bg-[#1e293b] border border-white/10 rounded-xl p-6 shadow-xl">
                                ...
                                <div className="space-y-3">
                                    {["Police Personnel", "Traffic Control Units", "Crowd Management Teams"].map((resourceName, index) => (
                                        <ResourceCheckbox key={index} category="OTHER_RESOURCES" resourceName={resourceName} index={index} />
                                    ))}
                                </div>
                            </div>
                        )}
